...
def _quality(**overrides):
    values = dict(min_sharpness=0.0, min_std=2.0, min_p1=0.0, max_p99=255.0)
...
def test_clean_reference_image_passes_and_is_traceable():
    result = inspect_array(_reference_image(), InspectionConfig(quality=_quality()))
...
def test_dark_defect_is_detected_with_metrology():
    ...
    result = inspect_array(
        image,
        InspectionConfig(quality=_quality(), min_area_px=100, max_area_px=2000),
    ...
    assert defect.equivalent_diameter_physical is not None
    assert defect.bbox_px[0] >= 65
    assert "max_defect_count_exceeded" in result.reject_reasons


def test_roi_restricts_inspection():
    image = _reference_image(120, 120)
    cv2.rectangle(image, (5, 5), (24, 24), (20, 20, 20), -1)
    result = inspect_array(image, InspectionConfig(quality=_quality(), roi=(40, 40, 60, 60)))
    assert result.passed
...
def test_quality_gate_rejects_flat_frame():
    ...
    assert "image_quality_gate" in result.reject_reasons


def test_bad_quality_can_be_overridden():
    image = np.full((100, 100, 3), 128, dtype=np.uint8)
    result = inspect_array(image, InspectionConfig(reject_bad_image_quality=False))
    assert result.passed
...
def test_shape_filter_exclusion_is_diagnostic_not_failure():
    image = _reference_image()
    cv2.rectangle(image, (55, 70), (145, 73), (10, 10, 10), -1)
    config = InspectionConfig(quality=_quality(), min_area_px=20, max_aspect_ratio=5.0)
    result = inspect_array(image, config)
    assert result.passed
...
def test_border_rejection_is_explicit():
    ...
    result = inspect_array(image, InspectionConfig(quality=_quality(), reject_border_touching=True))
    assert result.passed
    assert result.diagnostic_reasons
